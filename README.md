...
id只要无空格都行，但放开容易约束难，为了减少冲突，暂时不允许id中有.与*(现在允许有.，为了模仿js中多层字典访问，但两个点最好不能相邻)，使字典的展开访问更像js。但这可能破坏了使用者的统一风格，所以推荐自己克隆与重新实现，~~同时因为核心语法简单，我也没有心思与精力来维护一个统一的版本。~~ 几种模式没有交叉(即列表匹配中尚无内嵌列表匹配、字典匹配），因为不想让作用域变得太复杂难以分析，目前也不考虑类似js的eval。
